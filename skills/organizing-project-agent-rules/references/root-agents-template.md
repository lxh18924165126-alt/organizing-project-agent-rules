...
bootstrap 时先完成证据账本，只保留仓库可验证或用户确认的内容；单一代码模式不写成硬规则。只添加实际存在领域的路由，根只引用已创建的叶子。audit 时 validator 通过且无语义缺陷则保持 no-op。
...
- AgentHub 完整工作流仅在用户明确要求或已批准范围包含该工作流时加载；原生子代理按下节委派。
- 用户运行时配置、旧模型矩阵、上下文/压缩/价格阈值及不可见 rollout 字段外置留证；子代理调用参数遵循下节。
- Harness 仅在用户明确要求 Harness 或工程化工作流时加载；两者互不自动启用。
- 优先级：平台/沙箱/命令/CI > 当前用户范围 > 根内核 > 架构/安全/数据不变量 > 命中领域规则 > 可选工作流 > 仓库惯例。

## 子代理

- 两个或更多独立可并行且需实质检索/分析的分支，主动委派；仅简单、强顺序依赖或低并行收益时不调用，并在首条进度说明原因。
- 全部子代理用 `gpt-5.6-luna / max`，仅用户当前任务明确指定可改；按需及并发上限定数量，不固定为 3，避免重复工作与同文件并行写。
- 主代理等待全部子代理完成，核对证据和冲突后统一输出；子代理继承当前任务范围与授权边界。
