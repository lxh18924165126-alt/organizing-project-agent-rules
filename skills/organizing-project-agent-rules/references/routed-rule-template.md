...
- 子代理模型与推理强度：所有角色统一 `gpt-5.6-luna / max`，仅用户在当前任务中明确指定可覆盖；模型不可用时报告限制，不自动切换型号或强度。
- 分工：数量按任务需要与环境并发上限，不固定为 3；避免重复工作和并行修改同一文件。原生子代理的主动委派条件由根规定，不要求先启动 AgentHub。
- 汇总：主代理等待全部子代理完成，核对证据和冲突后统一输出；子代理继承当前任务范围与授权边界。
